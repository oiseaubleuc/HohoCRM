# Automatische updates (Sparkle) — Orion (macOS)

De native app **Orion** by HohohSolutions gebruikt **[Sparkle 2](https://sparkle-project.org/)**: op de achtergrond wordt je **appcast** (XML op **HTTPS**) geraadpleegd. Als er een nieuw zip-bestand staat met geldige handtekening, kan de gebruiker installeren vanuit de app (**Orion → Controleren op updates…** of `⇧⌘U`).
...
5. Upload `Orion-1.0.1.zip` en de bijgewerkte `appcast.xml` naar dezelfde basis-URL als in `SUFeedURL`.
