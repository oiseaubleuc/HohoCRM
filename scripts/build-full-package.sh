...
REL_NAME="Orion-FullPackage-v${VERSION}-${DATE_TAG}"
REL_DIR="$ROOT/releases/$REL_NAME"

APP_PKG="$ROOT/artifacts/Orion-v1.0.0.pkg"
APP_DMG="$ROOT/artifacts/Orion-v1.0.0.dmg"
NATIVE_APP="$ROOT/artifacts/Orion Native.app"
...
(
  cd "$REL_DIR"
  ditto -c -k --sequesterRsrc --keepParent "native/Orion Native.app" "installers/Orion-Native.app.zip" || true
  ditto -c -k --sequesterRsrc --keepParent "webapp" "webapp/Orion-webapp-dist.zip" || true
)
...
(
  ...
  if [ -f "webapp/Orion-webapp-dist.zip" ]; then
    shasum -a 256 "webapp/Orion-webapp-dist.zip" >> SHA256SUMS.txt
  fi
  if [ -f "installers/Orion-Native.app.zip" ]; then
    shasum -a 256 "installers/Orion-Native.app.zip" >> SHA256SUMS.txt
  fi
)

# 7) Customer readme
cat > "$REL_DIR/README-CUSTOMER.txt" <<TXT
Orion - Full Package v${VERSION}

Inhoud:
- installers/Orion-v1.0.0.pkg
- installers/Orion-v1.0.0.dmg
- native/Orion Native.app
...
4) Optioneel: installers/Orion-Native.app.zip (zelfde techniek als in de .pkg)
